...
void parse_ipv6(const unsigned char *ip_buffer, size_t remaining_size)
{
    ...
    if (inet_ntop(AF_INET6, &ip6_h->ip6_src, src_ip, sizeof(src_ip)) == NULL) {
        perror("inet_ntop");
        return;
    }

    if (inet_ntop(AF_INET6, &ip6_h->ip6_dst, dst_ip, sizeof(dst_ip)) == NULL) {
        perror("inet_ntop");
        return;
    }
    ...
    switch (next_header) {
        case 58:
            parse_icmp6(ip_buffer, remaining_size);
            break;
        case 17:
            parse_udp6(ip_buffer, remaining_size);
            break;
        case 6:
            parse_tcp6(ip_buffer, remaining_size);
            break;
        default:
            printf("Unknown next header\n");
            break;
    }
}
...
void print_ipv6(uint8_t version, uint8_t traffic_class, uint32_t flow_label,
                uint16_t payload_len, const char *next, uint8_t hop_limit,
