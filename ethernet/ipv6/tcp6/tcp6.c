...
void parse_tcp6(const unsigned char *ip_buffer, size_t remaining_size){
    ...
    if(remaining_size < ipv6_len){
        printf("Incomplete TCP6 packet");
        return;
    }

    size_t tcp6_size = remaining_size - ipv6_len;

    if (tcp6_size < sizeof(struct tcphdr)) {
        printf("Incomplete TCP header\n");
        return;
    }

    const struct tcphdr* tcph = (const struct tcphdr *)(ip_buffer + ipv6_len);
